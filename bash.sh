...
non_new=$(for entry in $(echo $version_list | sed 's/ /\n/g' | grep -oE "^[[:digit:]]+\.[[:digit:]]+" | sort | uniq) ; do echo $version_list | sed 's/ /\n/g' | grep -Fe "$entry" | sed 's/,.*//g' |  sort -V |  tail -1 ; done)
